...
import { useGallery, type GalleryThumbnail } from '@/hooks/useGallery';
...
/* ─── Thumbnail Card with variant carousel ─── */
function ThumbnailCard({
  ...
  const hasMultiple = thumb.variants.length > 1;
  const variant = thumb.variants[activeIdx] ?? thumb.variants[0];
  const imageUrl = variant?.imageUrl;

  const prev = useCallback(
    (e: React.MouseEvent) => {
      e.stopPropagation();
      setActiveIdx((i) => (i === 0 ? thumb.variants.length - 1 : i - 1));
    },
    [thumb.variants.length]
  );
  const next = useCallback(
    (e: React.MouseEvent) => {
      e.stopPropagation();
      setActiveIdx((i) => (i === thumb.variants.length - 1 ? 0 : i + 1));
    },
    [thumb.variants.length]
  );
    ...
    >
      ...
      >
        {imageUrl ? (
          <img
            src={imageUrl}
            alt={`${thumb.title} — variant ${activeIdx + 1}`}
        ...
        {/* ◀ ▶  Arrows — visible on hover when >1 variant */}
        {hasMultiple && (
          <>
            <button
              onClick={prev}
              className="absolute left-1.5 top-1/2 -translate-y-1/2 z-10 flex h-7 w-7 items-center justify-center rounded-full bg-black/50 backdrop-blur-sm text-white/80 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/70"
            ...
            <button
              onClick={next}
              className="absolute right-1.5 top-1/2 -translate-y-1/2 z-10 flex h-7 w-7 items-center justify-center rounded-full bg-black/50 backdrop-blur-sm text-white/80 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/70"
          ...
          {imageUrl && (
            <button
              onClick={() =>
                downloadImage(
                  imageUrl,
                  `${thumb.title.replace(/\s+/g, '-')}-v${activeIdx + 1}.png`
                )
              }
        ...
        {hasMultiple && (
          <span className="absolute top-2 left-2 text-[10px] font-semibold bg-black/60 backdrop-blur-sm px-2 py-0.5 rounded-full text-white/80">
            {activeIdx + 1}/{thumb.variants.length}
          </span>
        )}
...
export default function GalleryPage() {
  ...
  const { thumbnails, loading, error, hasMore, fetchMore, deleteThumbnail, toggleFavourite, refetch } =
