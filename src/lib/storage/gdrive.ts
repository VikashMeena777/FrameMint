...
export async function uploadToGDrive(
  ...
  // Get a shareable link — retry with delay since GDrive may need
  // a moment to index the newly uploaded file
  ...
  for (let attempt = 1; attempt <= MAX_LINK_RETRIES; attempt++) {
    try {
      ...
      if (attempt < MAX_LINK_RETRIES && msg.includes('not found')) {
        console.warn(`[rclone] link attempt ${attempt}/${MAX_LINK_RETRIES} failed (not indexed yet), retrying in 2s...`);
  ...
  throw new Error(`rclone link failed after ${MAX_LINK_RETRIES} attempts for ${remotePath}`);
}
